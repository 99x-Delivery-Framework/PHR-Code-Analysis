...
cli_token=$(curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "name=${unique_token}" -u admin:admin http://localhost:${SONARQUBE_PORT}/api/user_tokens/generate | grep -o '"squ[^"]*"' | awk -F\" '{print $2}')
